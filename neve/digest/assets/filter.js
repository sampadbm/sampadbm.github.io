...
class FilterManager {
    ...
    buildFilterCheckboxGroup(filterName, options, label) {
        ...
        const limitDisplay = options.length > 5; // Collapse if more than 5 options
        ...
        return `
            <div class="filter-group" data-filter-type="${filterName}">
                ...
                <div class="filter-options ${limitDisplay ? 'collapsed' : ''}" data-filter-options="${filterName}">
                    ${options.map(opt => {
        ...
        container.querySelectorAll('.filter-toggle').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                if (optionsContainer) {
                    ...
                    e.target.textContent = optionsContainer.classList.contains('collapsed') ? '▼' : '▲';
                }
            });
        });
